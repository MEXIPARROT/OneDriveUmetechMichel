...
namespace ConsoleLib
{
    public class OneDriveClient
    {
        ...
        public async Task<string> Upload(string userID, string FolderID, Stream stream, string Name) //expect DriveItem?
        {
            ...
            else//more than 4MB/4 Million Bytes
            {
                //DriveItem resultDriveItem = null;

                var uploadSession = await _client.Users[userID].Drive.Items[FolderID]
                    .ItemWithPath(Name)
                    .CreateUploadSession()
                    .Request()
                    .PostAsync();

                var maxSlice = 320 * 1024;
                //Console.WriteLine(uploadSession.AdditionalData.Keys);
                //var largeFileUpload = new LargeFileUploadTask<>
                var largeFileUpload = new LargeFileUploadTask<DriveItem>(uploadSession, stream, maxSlice); //LargeFileUpload<DriveItem>(uploadSession, stream, maxSlice);

                UploadResult<DriveItem> uploadResult = await largeFileUpload.UploadAsync();
